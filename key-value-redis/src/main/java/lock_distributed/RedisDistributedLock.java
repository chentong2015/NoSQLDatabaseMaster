...
public class RedisDistributedLock {
    ...
    public String testDistributedLockPlus() {
        ...
        // 将两个操作合并到一个原子操作中
        boolean isGetLocked = stringRedisTemplate.opsForValue()
                .setIfAbsent(key, "myValue", 10, TimeUnit.SECONDS);
        if (!isGetLocked) {
            return "Error";
        }
        try {
    ...
    //                   3S   3S
    // 场景分析:
    // 由于设置了锁的过期事件，在业务的执行时间和锁的过期时间不一致时
    // 在A请求的业务执行没有结束时，锁被释放，这时B线程能够获得锁，开始执行
    ...
    public String testDistributedLockPlus2() {
        ...
        Boolean isGetLocked = stringRedisTemplate.opsForValue()
                .setIfAbsent(key, threadIdValue, 10, TimeUnit.SECONDS);
